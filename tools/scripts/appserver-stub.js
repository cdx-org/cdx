...
import { createLspMessageReader, writeLspMessage } from './lsp.js';
...
function buildPlan() {
  ...
  const buildParallel = count => ({
    tasks: Array.from({ length: count }, (_, index) => {
      const id = namePool[index] ?? `task-${index + 1}`;
  ...
  const buildSerial = count => ({
    tasks: Array.from({ length: count }, (_, index) => {
      const id = namePool[index] ?? `task-${index + 1}`;
      const label = id.charAt(0).toUpperCase() + id.slice(1);
      const dependsOn = index === 0 ? [] : [namePool[index - 1] ?? `task-${index}`];
      return { id, description: `${label} task`, dependsOn, prompt: `Do ${id}.` };
    }),
  });
...
function isTaskPrompt(text) {
  if (!text) return false;
  return (
    text.includes('Task assigned to you (') ||
...
reader.onMessage(async message => {
  ...
  if (method === 'turn/start') {
    ...
    const plannerPrompt = isPlannerPrompt(inputText);
    const taskPrompt = !plannerPrompt && (taskThreadIds.has(threadId) || isTaskPrompt(inputText));
    if (plannerPrompt) plannerTurnCounter += 1;
    if (taskPrompt) taskThreadIds.add(threadId);
    const delayMs = resolveDelayMs({ inputText, isPlanner: plannerPrompt });
